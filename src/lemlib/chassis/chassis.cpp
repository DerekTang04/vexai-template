...
lemlib::OdomSensors::OdomSensors(OdometryPod *hPod, OdometryPod *vPod, CalibratedImu *imu, CalibratedGps *gps): 
    hPod(hPod), vPod(vPod), imu(imu), gps(gps) {}

lemlib::Drivetrain::Drivetrain(pros::MotorGroup* leftMotors, pros::MotorGroup* rightMotors, float trackWidth, float horizontalDrift) : 
    leftMotors(leftMotors), rightMotors(rightMotors), trackWidth(trackWidth), horizontalDrift(horizontalDrift) {}
...
void lemlib::Chassis::calibrate(Pose initialPose) {
    // zero rotation sensors
    sensors.hPod->calibrate();
    sensors.vPod->calibrate();

    // calibrate imu
    sensors.imu->calibrate(initialPose.theta);

    // calibrate gps
    sensors.gps->calibrate(initialPose.x, initialPose.y, initialPose.theta);

    // set odometry sensors
    setSensors(sensors);
    // set odometry pose
    setPose(initialPose);
    // start odometry task
